...
def carousel_data():
    return [
        dict(image_url="static/images/black_widow.jpg", label="Black widow", active="active"),
        dict(image_url="static/images/blackpanther.jpg", label="Black Panther"),
        dict(image_url="static/images/doctorstrange.jpg", label="Doctor Strange"),
        dict(image_url="static/images/ironman.jpg", label="Iron Man"),
        dict(image_url="static/images/spiderman.jpg", label="Spider Man"),
        dict(image_url="static/images/thor.jpg", label="Thor"),
    ]
...
def tabs_data():
    ...
    def create_tabs():
        superheroes = Superhero.objects.all()  # Fetch superheroes from the database
        tabs = []
        for superhero in superheroes:
            data = card_data(
                title=superhero.name,  # Use superhero name as the title
                body=f"This is a card about {superhero.name}. They go by {superhero.identity}, and is the {superhero.description}. Their strengths are {superhero.strengths} and their weaknesses are {superhero.weaknesses}. Image: {superhero.image}  ",  # Populate with superhero details
                link=dict(href='Home', text='Home')
            )
            data['cards'] = [data]  # Include the card in the tab's data
            tabs.append(data)
        return tabs
